...
var homeLoanCalMod = (function () {
	function homeLoanCalMod(o) {
	...
	_createClass(homeLoanCalMod, [{
		key: "updatePaid",
		value: function updatePaid(p) {
			this.paid += p;
		}
	}, {
		key: "getIntrThisMonth",
		value: function getIntrThisMonth(currentSaving) {
		...
		value: function cal() {
			...
			while (this.paid < this.totalMog) {
				...
				this.result.push([month, Math.round(currentSaving * 100) / 100, Math.round(thisMonthInterest * 100) / 100, Math.round(currentPaid * 100) / 100, Math.round(currentRemaining * 100) / 100]);
			}
		}
	}, {
		key: "dis",
